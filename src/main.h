...
static const int LAST_POW_BLOCK = 4790666;
...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 60 * CENT;	// 60% annual interest
static const int64_t DEF_SPLIT_AMOUNT = 20000 * COIN;  // Split Threshold Default
static const int64_t MAX_SPLIT_AMOUNT = 100000 * COIN;  // Split Threshold Max
...
public:
    ...
    {
        // version: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // version: get transaction coin age
...
public:
    ...
    // version: block signature - signed by coin base txout[0]'s owner
    ...
    // version: two types of block: proof-of-work or proof-of-stake
    ...
    // version: get max transaction timestamp
    ...
    bool ConnectBlock(int nHeight, CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck=false);
    bool ReadFromDisk(const CBlockIndex* pindex, bool fReadTransactions=true);
    bool SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew);
    bool AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos);
    bool CheckBlock(bool fCheckPOW=true, bool fCheckMerkleRoot=true, bool fCheckSig=true) const;
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // version: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFlags;  // version: block index flags
    enum  
    {
