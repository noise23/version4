    ...
    model(0)
{
    ...
    /* Do not move this to the XML file, Qt before 4.7 will choke on it */
     ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a V address (e.g. VJn5GfLLQLPSjraCF7fFH7yYvN2dy9GwUa)"));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
		...
		{ 
#if QT_VERSION < 0x050000 
		formatted.append(tr("<b>%1</b> in %4 blocks of %5 TRK each to %2 (%3)?").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount),  
			Qt::escape(rcp.label),  
			rcp.address,  
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid V address"));
        }
        else
        {
